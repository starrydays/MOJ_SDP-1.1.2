...
cost_test=churn_test;
    ...
    result_test=A_test*xx(1,2:11)'+xx(1,1);%769
    pred_test=Logistic(result_test);
    WPdata_test(:,1) = test_data(:,14);%bug
    WPdata_test(:,2) = churn_test;%churn
    WPdata_test(:,3) = ifelse(pred_test>0.5,1,0);%ifelse(pred>0.5,1,0);%pred
    WPdata_test(:,4) = WPdata_test(:,1)./(WPdata_test(:,2)+1);%density
